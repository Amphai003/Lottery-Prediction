...
func getOpenAIKey() string { return os.Getenv("OPENAI_API_KEY") }
func getGeminiKey() string { return os.Getenv("GEMINI_API_KEY") }
...
func CallOpenAI(history string, digits string, count string) string {
	...
	req.Header.Set("Content-Type", "application/json"); req.Header.Set("Authorization", "Bearer "+getOpenAIKey())
...
func CallGemini(history string, digits string, count string) string {
	...
	data, _ := json.Marshal(payload); url := "https://generativelanguage.googleapis.com/v1beta/" + m + ":generateContent?key=" + getGeminiKey()
